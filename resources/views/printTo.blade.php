...
<body>
    <div style="margin: 0 auto;display: block;width: 700px;">
        ...
        <main>
            ...
            <table class="table" width="100%" border="1">
                <tbody>
                    ...
                    foreach($tr[0]['stdetails'] as $key => $x) { 
                        $serialNumbers = $tr_details[$key]->serial_numbers;
                        ?>
            ...
            <br>
            <?php if ($serialNumbers !== null): ?>
            <hr>
                        <!-- Separate table for item codes and serials -->
            ...
            <table class="table" width="100%" border="1">
                <tbody>
                    ...
                    <?php foreach ($tr[0]['stdetails'] as $key => $x): ?>
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>{{ $x['item']['item_no'] }}</td>
                            <td>
                                ...
                                // Iterate through $tr_details to find the matching item
                                foreach ($tr_details as $trDetail) {
                                    if ($trDetail->item_id == $x['item']['id']) {
                                        $serialNumbers = $trDetail->serial_numbers;
                                        $itemSerials = $serialNumbers ? explode(',', $serialNumbers) : [];
                                        break; // Stop searching once a match is found
                                    }
                                }
            
                                echo implode(', ', $itemSerials) ?: '-'; // Display serials or dash if empty
                                ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
